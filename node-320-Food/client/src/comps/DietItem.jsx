...
const DietItem = ({ food }) => {
  /*
    식품명 표시된 곳을 click했을 때 input box가 나타나도록 하려고 한다
    1. DietList에서 isEdit state 변수를 만들고 item을 생성하면서
        isEdit 변수의 상태에 따라 input box 또는 그냥 문자열이 나타나도록
        하였다
        ==> 모든 item이 isEdit 상태를 공유하게 되어 한개의 항목을 클릭하면 모든 항목에 input box가 나타났다
    2. DietItem을 별도로 분리하고 DietItem에서 isEdit 상태변수를 생성한 후 같은 코드로 input box가 나타나도록 하였다
        똑같은 이름의 isEdit 이지만 각 item마다 서로 다른 isEdit를 만들게 되어 클릭한 item만 input box가 나타나도록 변경된다
    */
