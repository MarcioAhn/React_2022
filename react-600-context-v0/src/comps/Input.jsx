// Main 으로부터 전달받은 변수를 받기 위해 props를 선언
const Input = (props) => {
  // props로 부터 address state 변수를 분해하기
  const { address, setAddress, addrList, setAddrList } = props;
  // address state 변수로부터 각각 속성 요소를 분해하기
  ...
  /*
    ...
    value 속성에 연결된 변수(state, 일반)의 값을 직접 변경하는
    성질이 발생한다
    react에서는 state변수를 임의로 변경하는 것이 금지되어 있다
    이러한 특성을 유지하기 위하여 value={변수} 형태로 할당이 되면
    쓰기금지(read only) 성질이 발생한다

    이럴 때 defaultValue를 사용하여 변수를 할당할 수도 있지만
    간혹 defaultValue로 설정된 변수값은 state의 특성을
    제대로 발휘 하지 못한다 그래서 defaultValue보다는 value 속성을 사용

    input box에 문자열을 키보드로 입력하면
    input box의 onChange event를 사용하여 setState() 함수를 통하여
    state 변수에 값을 변경하도록 하는 코드를 작성해야 한다

    input box에 입력된 문자열을 onChange 이벤트가 가로채고
    가로챈 문자열을 setState 함수를 통하여 state 변수를 변경하고
    state 의 rendering 특성에 따라 input box에 문자열이
    나타나는 코드가 된다
  */
  const onChangeEvent = (e) => {
    ...
    각 input box가 공통으로 사용할 수 있는 코드 작성이 가능하다
    */
    // [name] = value;
    // 다음의 코드는 각각 input box의 event에 따라서
  ...
  /*
  저장버튼을 클릭하면
  address에 저장된 데이터를 addrList 추가하기
